...
argocd-applicationset-controller          ClusterIP   192.168.126.223   <none>        7000/TCP,8080/TCP            54s
argocd-dex-server                         ClusterIP   192.168.126.118   <none>        5556/TCP,5557/TCP,5558/TCP   54s
argocd-metrics                            ClusterIP   192.168.126.238   <none>        8082/TCP                     54s
argocd-notifications-controller-metrics   ClusterIP   192.168.126.170   <none>9001/TCP                     54s
argocd-redis                              ClusterIP   192.168.126.55    <none>        6379/TCP                     54s
argocd-repo-server                        ClusterIP   192.168.126.188   <none>        8081/TCP,8084/TCP            54s
argocd-server                             ClusterIP   192.168.126.147   <none>        80/TCP,443/TCP               54s
argocd-server-metrics                     ClusterIP   192.168.126.89    <none>        8083/TCP                     54s
...
spec:
  ...
  rules:
  - host: argocd.20.249.165.109.nip.io
    http:
      paths:
      - backend:
          service:
...
# 확인
$ curl https://argocd.20.249.165.109.nip.io/ -i -k
HTTP/2 200
...
{"level":"fatal","msg":"Argo CD server address unspecified","time":"2025-10-19T04:47:35Z"}
...
# Login ArgoCD
$ argocd login argocd.20.249.165.109.nip.io
Username: admin
Password: wsNLmL9P4dpPJmqr


# [참고] Login to Argo CD using SSO
$ argocd login argocd.20.249.165.109.nip.io --sso

# [참고] Configure direct access using Kubernetes API server
$ argocd login argocd.20.249.165.109.nip.io --core


# logout
$ argocd logout argocd.20.249.165.109.nip.io
Logged out from 'argocd.20.249.165.109.nip.io'
...
# 비밀번호 update - login 후에 실행가능
$ argocd account update-password \
    --account admin \
    --current-password wsNLmL9P4dpPJmqr \
    --new-password New1234! \
    --grpc-web
Password updated
Context 'argocd.20.249.165.109.nip.io' updated
...
* argocd 내 통신하는 두가지 방식
  * gRPC  (기본) : CLI가 TCP 기반으로 통신(443 포트 이용) 
  * gRPC-web : http 기반 gRPC 통신
...
# Cluster 추가
$ argocd cluster add ktdsedu-aks
...
```sh
argocd UI내 Deployment 에서 Live Manifest 부분에서 아래 에러 발생.

Resource not found in cluster: apps/v1/Deployment:samplejava

```

**원인**

* ArgoCD의 argro-server ServiceAccount에 권한이 없어서 발생
...
```sh
$ kubectl -n argocd-system edit cm argocd-cm -o yaml
apiVersion: v1
data:
...
```sh
$ kubectl -n argocd get clusterrole argocd-server -o yaml


$ kubectl -n argocd edit clusterrole argocd-server
...
Argo CD는 kubectl을 통해 직접 Application 리소스를 생성해 GitOps를 선언적으로 구성할 수 있다.
...
$ cat <<EOF | kubectl -n argocd-system apply -
apiVersion: argoproj.io/v1alpha1
kind: Application
metadata:
  name: edu00-samplejava-dev               # Application 이름
spec:
  project: default                         # (기본 프로젝트 사용)
  source:
    repoURL: https://github.com/rohansong/SampleJava
    targetRevision: main                   # Git 브랜치
    path: manifests/overlays/dev           # kustomization.yaml 이 있는 경로
    kustomize: {}                          # kustomize 사용
  destination:
    server: https://kubernetes.default.svc # in-cluster 배포
    namespace: edu00-dev                   # 배포 대상 네임스페이스
  syncPolicy:
    automated:                             # 자동 동기화 옵션
...
$ kubectl -n argocd-system describe application samplejava-dev
